function [p,rh,E_rel,it]=rcubica(a,xa,xb)
  ...
  %  El objetivo es encontrar la raíz cúbica de a, es decir, un número x tal que:
  f = @(x) x.^3 -a;
  ...
  [p, rh, it, E_rel,tiempo] = biseccion(f, xa, xb, maxit, tol);
  ...
  hold on
  semilogy(rh,'r-*')
  legend('Error Relativo', 'Residuo', 'Location', 'best')
